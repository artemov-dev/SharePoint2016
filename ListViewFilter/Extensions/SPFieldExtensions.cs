...
namespace ListViewFilter.Extensions
{
    internal static class SPFieldExtensions
    {
        ...
        public static T MaxValue<T>(this SPField field)
        {
            var query = new SPQuery
                            {
                                ViewFieldsOnly = true,
                                ViewFields = string.Format(@"<FieldRef Name=""{0}"" />", field.InternalName),
                                RowLimit = 1,
                                Query = string.Format(@"<Where><IsNotNull><FieldRef Name=""{0}""/></IsNotNull></Where><OrderBy><FieldRef Name=""{0}"" Ascending=""FALSE"" /></OrderBy>", field.InternalName)
                            };
            var items = field.ParentList.GetItems(query);
            if (items.Count > 0)
            {
                return (T)(items[0][field.Id]);
            }
            return default(T);
        }

        public static T MinValue<T>(this SPField field)
        {
            var query = new SPQuery
            {
                ViewFieldsOnly = true,
                ViewFields = string.Format(@"<FieldRef Name=""{0}"" />", field.InternalName),
                RowLimit = 1,
                Query = string.Format(@"<Where><IsNotNull><FieldRef Name=""{0}""/></IsNotNull></Where><OrderBy><FieldRef Name=""{0}"" Ascending=""TRUE"" /></OrderBy>", field.InternalName)
            };
            var items = field.ParentList.GetItems(query);
            if (items.Count > 0)
            {
